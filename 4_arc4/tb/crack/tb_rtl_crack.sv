...
module tb_rtl_crack();
    ...
    // ============== Memory helper tasks =============
    // rtl tb so we will have helpers for direct memory accesses which adhere w the lab readme 
    ...
    // THE ACTUAL TESTS 
    initial begin
        $display("=== TB: RTL crack ===");

        en = 1'b0;
        rst_n = 1'b0;


        // keep the reset low first so DUT starts from clean state then load the test ciphertext inot CT memory 
        #10;
