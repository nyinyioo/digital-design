...
module tb_syn_task2();
    ...
    task read_mem(input [7:0] mem_addr, output [7:0] mem_data);
        begin
            mem_data = DUT.\s|altsyncram_component|auto_generated|altsyncram1|ram_block3a0 .ram_core0.ram_core0.mem[mem_addr];
        end
    endtask
    ...
    initial begin
        ...
        // Give init + ksa plenty of time to finish
        repeat(1500) @(posedge CLOCK_50);

        check_against_golden();

        // Check that the memory stays stable afterward
        repeat(20) @(posedge CLOCK_50);
        check_against_golden();

        // Reset again to make sure the top-level sequence restarts properly
        @(negedge CLOCK_50);
        KEY[3] = 1'b0;
        @(posedge CLOCK_50);
