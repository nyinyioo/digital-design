...
module tb_syn_doublecrack();
    ...
    // ============== Memory helper tasks =============
    // direct synthesized hierarchy access for the top-level CT memory
    ...
    task read_c1_pt(input [7:0] addr, output [7:0] data);
        begin
            data = DUT.\c1|pt|altsyncram_component|auto_generated|altsyncram1|ram_block3a0 .ram_core0.ram_core0.mem[addr];
        end
    endtask

    task read_c2_pt(input [7:0] addr, output [7:0] data);
        begin
            data = DUT.\c2|pt|altsyncram_component|auto_generated|altsyncram1|ram_block3a0 .ram_core0.ram_core0.mem[addr];
        end
    endtask
    ...
    initial begin
        ...
        // test 1
        // CODE vector should be found by the odd-key side so c2 should be the winning core
        ...
        // test 2
        // TEST vector should be found by the even-key side so c1 should be the winning core
        ...
        $display("=== tb_syn_doublecrack PASSED ===");
        $finish(0);
    end
