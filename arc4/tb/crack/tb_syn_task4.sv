...
module tb_syn_task4();
    ...
    // Test vector
    // same ciphertext used in syn_crack plain text word is "CODE"
    ...
    // task4 should keep the displays blank while computing and then eventually show the key
    task wait_for_task4_done;
        ...
        begin
            ...
            assert(cycles < 20000) else $fatal("timeout waiting for task4 displays to update");
        end
    endtask

    // Test stimulation
    initial begin
        $display("=== TB: SYN task4 ===");
        ...
        // displays should still be blank while we're still early in the search
        repeat(20) @(posedge CLOCK_50);
        #1;
        check_displays_blank();
        ...
        // apply reset again and make sure the top level reruns properly
        @(negedge CLOCK_50);
        KEY[3] = 1'b0;
        @(posedge CLOCK_50);
        #1;
        ...
        /*
            one small forced check for the failure display case

            this only touches top level outputs of crack as seen by task4
            and lets us still verify the dash display behavior in the synthesized bench
        */
